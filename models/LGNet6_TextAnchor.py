...
class LGNet6_TextAnchor(nn.Module):
    def __init__(self, k=1.5, c_in=40):
        super(LGNet6_TextAnchor, self).__init__()
    ...
    def forward(self, pos, neg, anchor):
        ...
        pos = self.LGBlock4(pos)
        pos = self.LGBlock5(pos)
        pos = self.LGBlock6(pos)
        pos = self.pool(pos)
        ...
        neg = self.LGBlock4(neg)
        neg = self.LGBlock5(neg)
        neg = self.LGBlock6(neg)
        neg = self.pool(neg)
