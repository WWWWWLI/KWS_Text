...
class LGNet3(nn.Module):
    def __init__(self, k=1, c_in=40):
        super(LGNet3, self).__init__()
        ...
        if config.TRAIN.MODE == 'FinetuneSilence':
            ...
            self.classifier = nn.Linear(52, config.TEXT_PROJ_DIM)
            self.classifier2 = nn.Linear(config.TEXT_PROJ_DIM, config.NumClasses)

        else:
            self.classifier = nn.Linear(52, config.NumClasses)

        # self.classifier = nn.Linear(int(48 * k), config.NumClasses)
